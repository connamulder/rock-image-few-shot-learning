"""
    ...
    @info   : Run the few-shot learning experiment.
"""
...
from KGTN.KGTN import KGTN
from KGTN.util import process_adjacent_matrix
    ...
    for i in range(maxiters):  
        ...
        if (i%1000==0):
            with h5py.File(params.trainfile, 'r') as f:
                ...
                with torch.no_grad():
                    eval_loop_step(test_loader, model, base_classes, novel_classes, tag='Training')
    return model
...
def eval_loop_step(data_loader, model, base_classes, novel_classes, tag='Testing'):
    ...
    print("**********************************{}**********************************".format(tag))
    print("novel in all  :  {:<8.2f}".format(top1_novel*100))
    print("novel in novel:  {:<8.2f}".format(top1_novel_new_metrix*100))
    print("all           :  {:<8.2f}".format(top1_all*100))
    print("all with prior:  {:<8.2f}".format(top1_all_with_prior*100))

    print("***************************************************************************")
    
    return np.array([top5_novel, top5_novel_new_metrix, top1_all, top1_all_with_prior])


def parse_args():
    parser = argparse.ArgumentParser(description='Low shot benchmark')
    parser.add_argument('--lowshotmeta', default='F:/Pytorch_learning/rock-image-few-shot-learning/data/label_idx_rocks_24-8.json', type=str, help='set of base and novel classes')
    parser.add_argument('--experimentpath', default='F:/Pytorch_learning/rock-image-few-shot-learning/data/splitfile_rock_train_{:d}.json', type=str, help='path of experiments')
    parser.add_argument('--experimentid', default=2, type=int, help='id of experiment')
    parser.add_argument('--lowshotn', default=20, type=int, help='number of examples per novel class')
    parser.add_argument('--trainfile', default='F:/Pytorch_learning/rock-image-few-shot-learning/data/VGG16_train.hdf5', type=str)
    parser.add_argument('--valfile', default='F:/Pytorch_learning/rock-image-few-shot-learning/data/VGG16_val.hdf5', type=str)
    parser.add_argument('--testfile', default='F:/Pytorch_learning/rock-image-few-shot-learning/data/VGG16_test.hdf5', type=str)
    parser.add_argument('--testsetup', default=1, type=int, help='test setup or validation setup?')
    parser.add_argument('--numclasses', default=32, type=int)
    parser.add_argument('--lr', default=0.1, type=float)
    parser.add_argument('--momentum', default=0.9, type=float)
    parser.add_argument('--wd', default=0.001, type=float)
    parser.add_argument('--l2_reg', default=0.001, type=float)
    parser.add_argument('--maxiters', default=11001, type=int)
    parser.add_argument('--batchsize', default=32, type=int)
    parser.add_argument('--outdir', default='F:/Pytorch_learning/rock-image-few-shot-learning/results/VGG16', type=str, help='output directory for results')
    parser.add_argument('--use_knowledge_propagation', default=True, action='store_true', help='whether use KGTN')
    parser.add_argument('--use_all_base', default=True, action='store_true', help='whether use all base category to do propagation')
    ...
    parser.add_argument('--process_type', default='semantic', type=str, help='output directory for results')
    parser.add_argument('--adjacent_matrix_file', default='F:/Pytorch_learning/rock-image-few-shot-learning/similarity.npy', type=str, help='adjacent matrix file')
    parser.add_argument('--kg_ratio', default=100, type=int, help='ratio of the kg information')
    parser.add_argument('--classifier_type', default='cosine', type=str, help='classifier_type')  # inner_product  cosine  pearson
    parser.add_argument('--matrix_type', default='All', type=str, help='classifier_type')    # OnlyMineral  All
...
if __name__ == '__main__':
    ...
    if params.use_all_base:
        train_base_classes = lowshotmeta['base_classes_2']   # lowshotmeta['base_classes_1'] +
    else:
    ...
    print('trained')
    with h5py.File(params.valfile, 'r') as f:
        val_loader = get_test_loader(f)
        with torch.no_grad():
            accs_val = eval_loop_step(val_loader, model, base_classes, novel_classes, tag='Validation')
    with h5py.File(params.testfile, 'r') as f:
        test_loader = get_test_loader(f)
        with torch.no_grad():
            accs_test = eval_loop_step(test_loader, model, base_classes, novel_classes)
    
    if not os.path.exists(params.outdir):
        os.makedirs(params.outdir)
        
    modelrootdir = os.path.basename(os.path.dirname(params.trainfile))
    outpath = os.path.join(params.outdir, modelrootdir+'_'+params.classifier_type+'_'+params.matrix_type+'_lr_{:.3f}_wd_{:.3f}_expid_{:d}_lowshotn_{:d}.json'.format(
                                    params.lr, params.wd, params.experimentid, params.lowshotn))
    respath = os.path.join(params.outdir, modelrootdir+'_'+params.classifier_type+'_'+params.matrix_type+'.txt')
    respath_novel = os.path.join(params.outdir,
                                 modelrootdir + '_' + params.classifier_type + '_' + params.matrix_type + '_novel.txt')
    with open(outpath, 'w') as f:
        json.dump(dict(lr=params.lr, wd=params.wd, expid=params.experimentid, lowshotn=params.lowshotn, accs=accs_val.tolist()), f)
        json.dump(dict(lr=params.lr, wd=params.wd, expid=params.experimentid, lowshotn=params.lowshotn, accs=accs_test.tolist()), f)
    with open(respath, 'a') as f:
        res_txt = '{:.3f}\n'.format(accs_test[2])
        f.write(res_txt)
    with open(respath_novel, 'a') as f:
        res_txt = '{:.3f}\n'.format(accs_test[0])
        f.write(res_txt)
